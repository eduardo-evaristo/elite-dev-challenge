...
The system SHALL expose an authenticated `GET /catalog/{type}/{externalId}` endpoint that returns the full normalized details for a single catalog entry from the external source mapped to `type`.

#### Scenario: Movie details by TMDB id
- **WHEN** an authenticated `ORGANIZER` requests `GET /catalog/movie/550`
- **THEN** the system returns a `CatalogItemDetail` from TMDB for movie id 550, including `genres`, `runtime`, and `tagline` when present

#### Scenario: Show details by TicketMaster id
- **WHEN** an authenticated `ADMIN` requests `GET /catalog/show/G5diZfkn0B-bh`
- **THEN** the system returns a `CatalogItemDetail` from TicketMaster for that event id, including `venue`, `city`, and `priceRange` when present
...
The frontend and backend SHALL share the catalog response types via a workspace package (`@elite-dev/shared`) exporting `CatalogItem`, `CatalogItemDetail`, `SearchParams`, and `PaginatedCatalogResult`. The backend SHALL use these types as its response contract; the frontend SHALL import them as type-only imports.
